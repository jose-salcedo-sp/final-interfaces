            ...
            <VStack space="2xl" className="mt-10">
                ...
                <VStack space="xs">
                            ...
                            <HStack className="items-center space-x-4">
                                ...
                                <Text className="text-right text-black bg-up-white rounded-full px-3 py-1 flex-1">
                                    {(user.finances.balance < 0 ? "- " : "") +
                                        "$" +
                                        Math.abs(user.finances.balance) +
                                        " MXN"}
                                </Text>
                            </HStack>
                            {user.finances.balance < 0 && (
                    ...
                    {user.finances.balance < 0 && (
                        <Button className="bg-up-red rounded-3xl">
                            <ButtonText size="xl">Pay Now!</ButtonText>
                        </Button>
                    )}
                </VStack>
                ...
                <Card className="bg-up-blue rounded-3xl p-6 space-y-4">
                    <VStack space="md">
                        ...
                        {user.finances.movements
                            .sort((a, b) => b.date.localeCompare(a.date))
                            .slice(0, account_movements_count)
                            .map((movement, key) => (
                                <HStack key={key}>
                                    <Heading
                                        size="sm"
                        ...
                        {account_movements_count <
                            user.finances.movements.length - 1 && (
                            <Button
                                onPress={() =>
                                    setAccountMovements(
                                        account_movements_count + 5,
                                    )
                                }
